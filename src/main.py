...
async def on_command_error(ctx:commands.Context, error: commands.CommandError):
  ...
  if isinstance(error, commands.CommandNotFound):
    ...
    await ctx.send(f'⚠ I do not have the required permission : {", ".join(error.missing_permissions)}')
  else:
    logger.error(f'✕ Error in command : {error}')
    await ctx.send(f'⚠ An error :{str(error)[:100]} occurred while processing the command')
